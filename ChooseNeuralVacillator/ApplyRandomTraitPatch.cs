...
namespace ChooseNeuralVacillator
{
    ...
    public sealed class ApplyRandomTraitPatch
    {
        // The instance for [SelectWindow] so it can be destroyed later
        private static GameObject SelectWindowInstance = new GameObject();
        private static readonly TextStyleSetting myStyle = PUITuning.Fonts.UILightStyle.DeriveStyle(
            18
        );
        ...
        public static bool Prefix(
            WorkerBase worker,
            bool ___IsConsumed,
            GeneShuffler.GeneShufflerSM.Instance ___geneShufflerSMI
        )
        {
            ...
            if (list.Count == 0)
            {
                InfoDialogScreen infoDialogScreen2 = (InfoDialogScreen)
                    GameScreenManager.Instance.StartScreen(
                        ScreenPrefabs.Instance.InfoDialogScreen.gameObject,
                        GameScreenManager.Instance.ssOverlayCanvas.gameObject
                    );
                string text2 = string.Format(
                    UI.GENESHUFFLERMESSAGE.BODY_FAILURE,
                    worker.GetProperName()
                );
                infoDialogScreen2
                    .SetHeader(UI.GENESHUFFLERMESSAGE.HEADER)
                    .AddPlainText(text2)
                    .AddDefaultOK();
                return false;
            }
            ...
            PDialog SelectWindow = new PDialog("SelectWindow") { Title = Strings.SelectTitle };
            SelectWindow.Body.Spacing = 20;
            PLabel label = new PLabel("Description")
            {
                Text = string.Format(Strings.SelectDescription, worker.GetProperName()),
                TextStyle = myStyle,
            };
            ...
            foreach (string trait in list)
            {
                SelectWindow.Body.AddChild(
                    new PButton(trait)
                    {
                        ...
                        OnClick = delegate(GameObject button)
                        {
                            TraitSelected(worker, ___IsConsumed, ___geneShufflerSMI, button.name);
                        },
                    }
                );
            }
            ...
            SelectWindow.Body.AddChild(
                new PButton("Random")
                {
                    Text = Strings.SelectRandom,
                    TextStyle = myStyle,
                    FlexSize = new Vector2(0.8f, 1),
                    OnClick = delegate(GameObject button)
                    {
                        RandomSelected(worker, ___IsConsumed, ___geneShufflerSMI, list);
                    },
                }
            );
        ...
        /// </summary>
        private static void TraitSelected(
            WorkerBase worker,
            bool ___IsConsumed,
            GeneShuffler.GeneShufflerSM.Instance ___geneShufflerSMI,
            string traitID
        )
        {
            ...
            InfoDialogScreen infoDialogScreen = (InfoDialogScreen)
                GameScreenManager.Instance.StartScreen(
                    ScreenPrefabs.Instance.InfoDialogScreen.gameObject,
                    GameScreenManager.Instance.ssOverlayCanvas.gameObject,
            ...
            string text = string.Format(
                UI.GENESHUFFLERMESSAGE.BODY_SUCCESS,
                worker.GetProperName(),
                trait.Name,
                trait.GetTooltip()
            );
        ...
        /// </summary>
        private static void RandomSelected(
            WorkerBase worker,
            bool ___IsConsumed,
            GeneShuffler.GeneShufflerSM.Instance ___geneShufflerSMI,
            List<string> list
        ...
        /// </summary>
        private static void SetConsumed(
            bool consumed,
            GeneShuffler.GeneShufflerSM.Instance ___geneShufflerSMI
        )
        {
            ___geneShufflerSMI.master.IsConsumed = consumed;
            ___geneShufflerSMI.sm.isCharged.Set(
                !___geneShufflerSMI.master.IsConsumed,
                ___geneShufflerSMI
            );
        }
    }
